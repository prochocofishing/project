const pt = {
  "nav.home": "Início",
  "nav.products": "Produtos",
  "nav.about": "Sobre",
  "nav.contact": "Contacto",

  "hero.subtitle": "A maioria das grandes histórias nasce de uma ideia simples… e a nossa não foi diferente. Tudo começou com uma paixão profunda pelo mar e pela arte da pesca. Acreditávamos, com uma convicção inabalável, que era possível transformar esse amor em algo maior: criar, equipamento de pesca de elevada qualidade, capaz de resistir ao uso duro, servir quem vive o mar e, acima de tudo, ser verdadeiramente eficaz. Munidos do nosso conhecimento, dos melhores materiais e de uma dedicação sem limites, lançámo-nos ao desafio. E foi assim que uma simples paixão se tornou numa missão e numa grande história que continua a crescer.",

  "hero.button.view": "Ver Catálogo",
  "hero.button.wholesale": "Obter Preços por Grosso",

  "feature.professional.title": "Qualidade Profissional",
  "feature.professional.desc": "Concebido para pescadores exigentes",
  "feature.proven.title": "Resultados Comprovados",
  "feature.proven.desc": "Garantia de maior taxa de captura",
  "feature.wholesale.title": "Preços por Grosso",
  "feature.wholesale.desc": "Tabelas competitivas para retalhistas",

  "products.title": "Catálogo de Produtos",
  "title.home": "Início",
  "title.privacy": "Política de Privacidade",
  "title.terms": "Termos de Serviço",
  "title.cookies": "Política de Cookies",
  "products.cta.title": "Pronto para Ter os Nossos Produtos?",
  "products.cta.text": "Contacte-nos para preços por grosso e quantidades mínimas de encomenda",
  "products.cta.button": "Obter Preços por Grosso",

  "product.viewDetails": "Ver Detalhes",
  "product.spec.size": "Tamanho:",
  "product.spec.weight": "Peso:",
  "product.spec.sinking": "Afundamento:",
  "product.colorsLabel": "Cores Disponíveis:",
  "product.category.squid": "Toneira \"palhaço\" com chumbo visível",
  "product.feature.bkk": "Agulhas premium BKK",
  "product.feature.glow": "Fita que brilha no escuro",
  "product.feature.glowBody": "Corpo que brilha no escuro",
  "product.feature.rainbow": "Corpo arco-íris",
  "product.feature.goldBody": "Corpo dourado",
  "product.feature.redBody": "Corpo vermelho",
  "product.feature.purpleBody": "Corpo roxo",
  "product.feature.laser": "Olhos Laser",
  "products.retailers.label": "Disponível em:",
  "product.description.rainbow.question": "Quando usar um palhaço \"rainbow\"? (folha arco-íris)",
  "product.description.rainbow.line1": "Durante o dia, sobretudo quando o sol já está mais alto.",
  "product.description.rainbow.line2": "É um padrão muito versátil, funcionando bem em vários tipos de água, desde mais clara até ligeiramente turva.",
  
  "footer.products.title": "Produtos",
  "footer.products.list": ["Série PREDADOR","CIRCUS 3.0","CIRCUS 2.5","PINTAS 3.0","PINTAS 2.5"],
  "footer.company.title": "Empresa",
  "footer.company.list": ["A Marca","Contacto","Vendas por Grosso","Made in Portugal","Sobre nós"],
  "footer.support.title": "Apoio",
  "footer.contact.title": "Contacto",
  "footer.contact.email": "prochocofishing@gmail.com",
  "footer.contact.address": "Portugal, Setúbal",
  "footer.copyright": "© {year} PROCHOCO. Todos os direitos reservados.",
  "footer.privacy": "Política de Privacidade",
  "footer.terms": "Termos de Serviço",
  "footer.cookies": "Política de Cookies"

  ,"about.title": "Sobre a PROCHOCO",
  "about.paragraph": "A PROCHOCO é uma marca Portuguesa que desenha e comercializa palhaços para pesca de cefalópodes de alta qualidade com grande atenção ao pormenor. ",
  "about.value1.desc": "Design e produção 100% nacionais, com grande atenção ao pormenor.",
  "about.value2.title": "Qualidade Premium",
  "about.value2.desc": "Materiais duráveis e acabamento exigente para uso profissional.",
  "about.value3.title": "Eficácia Comprovada",
  "about.value3.desc": "Equilíbrio perfeito, movimento natural e cores altamente atrativas.",
  "about.stat1.number": "100%",
  "about.stat1.label": "Portuguesa",
  "about.stat2.number": "2",
  "about.stat2.label": "Tamanhos (2.5 / 3.0)",
  "about.stat3.number": "6",
  "about.stat3.label": "Padrões da série PREDADOR",
  "about.certs.title": "Tecnologias",
  "about.cert.bkk": "Agulhas BKK",
  "about.cert.glow": "Fita Glow",
  "about.cert.laser": "Olhos Laser",
  "about.cert.rainbow": "Arco-íris",

  "contact.title": "Contacto / Revendedores",
  "contact.subtitle": "Parcerias / Revendedores",
  "contact.name": "Nome",
  "contact.store": "Nome da Loja",
  "contact.email": "Email",
  "contact.message": "Mensagem",
  "contact.send": "Enviar Pedido",
  "contact.sending": "A enviar...",
  "contact.thanks": "Obrigado! Respondemos dentro de 24 horas.",
  "contact.error": "Erro ao enviar. Tente novamente mais tarde."

  ,"consent.title": "Privacidade",
  "consent.description": "Utilizamos cookies próprios e de terceiros para lhe oferecer uma melhor experiência e serviço. Para saber quais cookies usamos e como os desativar, leia a Política de Cookies. Para continuar, por favor escolha 'Aceitar' ou 'Recusar'.",
  "consent.policy_link_text": "Ler Política de Cookies",
  "consent.accept": "Aceitar",
  "consent.decline": "Recusar"
  ,"footer.optout": "Desativar Analytics"
  ,"privacy.title": "Política de Privacidade",
  "privacy.intro": "Nesta página explicamos que dados recolhemos, porque o fazemos e como podes contactar-nos.",
  "privacy.dataCollectedTitle": "Que dados recolhemos",
  "privacy.dataCollected": "Podemos recolher dados de navegação, cookies (analytics) e informações que nos forneces quando contactas via formulário.",
  "privacy.contactTitle": "Contacto",
  "privacy.contact": "Se tiveres questões sobre privacidade, escreve para prochocofishing@gmail.com"

  ,"terms.title": "Termos de Serviço",
  "terms.lastUpdated": "Última atualização: Novembro 2025",
  "terms.acceptanceTitle": "Aceitação dos Termos",
  "terms.acceptance": "Ao aceder e usar este site, aceitas estes termos de serviço. Se não concordas, por favor não utilizes o site.",
  "terms.useTitle": "Uso Permitido",
  "terms.use": "Este site é fornecido apenas para informação. Não podes usar este site para fins ilegais ou ilícitos, ou de qualquer forma que viole os direitos de outros.",
  "terms.ipTitle": "Direitos de Propriedade Intelectual",
  "terms.ip": "Todo o conteúdo neste site (texto, imagens, logótipos, etc.) é propriedade da PROCHOCO. Não podes reproduzir, distribuir ou transmitir conteúdo sem autorização escrita.",
  "terms.liabilityTitle": "Limitação de Responsabilidade",
  "terms.liability": "A PROCHOCO não será responsável por danos diretos, indiretos ou consequentes resultantes do uso ou incapacidade de usar este site.",
  "terms.contactTitle": "Contacto",
  "terms.contact": "Para questões sobre estes termos, contacta-nos em prochocofishing@gmail.com"

  ,"cookies.title": "Política de Cookies",
  "cookies.lastUpdated": "Última atualização: Novembro 2025",
  "cookies.whatAreTitle": "O que são cookies?",
  "cookies.whatAre": "Cookies são pequenos ficheiros de texto armazenados no teu navegador. Usamos cookies para melhorar a experiência do utilizador e recolher informações analíticas.",
  "cookies.whyUseTitle": "Por que usamos cookies?",
  "cookies.whyUse": "Utilizamos cookies para entender como os utilizadores interagem com o site, rastrear o desempenho e melhorar os nossos serviços.",
  "cookies.typesTitle": "Tipos de Cookies",
  "cookies.types": "Google Analytics: Recolhemos dados de navegação de forma anónima. Apenas carregamos este cookie após o teu consentimento explícito.",
  "cookies.consentTitle": "Consentimento",
  "cookies.consent": "Não carregamos cookies de analytics sem o teu consentimento prévio. Podes desativar analytics a qualquer momento clicando em 'Desativar Analytics'.",
  "cookies.contactTitle": "Contacto",
  "cookies.contact": "Se tiveres questões sobre cookies, escreve para prochocofishing@gmail.com"
};

export default pt;

